...
public class MyTimePickerView extends View
{
     ...
     * 选择选中的内容
     */
    public void setSelected2(String mSelectItem) {
        for (int i = 0; i < mDataList.size(); i++) {
            if (mDataList.get(i).substring(0,mDataList.get(i).length()-1).equals(mSelectItem)) {
                setSelected(i);
                break;
            }
        }
    ...
    private void init() {
        ...
        //被选中的数字颜色
        mPaint.setColor(ContextCompat.getColor(context, R.color.alpha_80_black));
        //第二个paint
        nPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        nPaint.setStyle(Style.FILL);
        nPaint.setTextAlign(Align.CENTER);
        //未选中的数字颜色
        nPaint.setColor(ContextCompat.getColor(context, R.color.color_B0B0B0));
    }
