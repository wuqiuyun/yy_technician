...
public final class BeepManager implements MediaPlayer.OnCompletionListener,
    ...
    public synchronized void updatePrefs() {
        ...
        if (playBeep && mediaPlayer == null) {
            ...
            if (!ActivityUtil.isActivityFinished(activity.get())) {
                activity.get().setVolumeControlStream(AudioManager.STREAM_MUSIC);
                mediaPlayer = buildMediaPlayer(activity.get());
            }
        }
    }
    ...
    public synchronized void playBeepSoundAndVibrate() {
        ...
        if (vibrate) {
            if (!ActivityUtil.isActivityFinished(activity.get())) {
                Vibrator vibrator = (Vibrator) activity.get()
    ...
    private MediaPlayer buildMediaPlayer(Context activity) {
        ...
        // 配置播放资源
        try {
            AssetFileDescriptor file = activity.getApplicationContext().getResources()
